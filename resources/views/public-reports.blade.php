...
<head>
    ...
    <title>DG SKIN & HAIR CLINIC</title>
...
<body class="bg-gray-50 min-h-screen">
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-lg text-gray-600">Mobile Number: <span
                    class="font-semibold text-blue-600">{{ $mobileNo }}</span></p>
            <div class="mt-4">
                <a href="/"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div
                class="report-card bg-white rounded-xl shadow-lg p-6 border border-gray-200 {{ $report->downloaded_at ? 'ring-2 ring-green-200 bg-green-50' : '' }}">
                <!-- Report Header -->
                <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center">
                        <div
                            class="w-12 h-12 {{ $report->downloaded_at ? 'bg-green-100' : 'bg-blue-100' }} rounded-lg flex items-center justify-center mr-3">
                            <i
                                class="fas fa-file-pdf {{ $report->downloaded_at ? 'text-green-600' : 'text-blue-600' }} text-xl"></i>
                        </div>
                        <div>
                            <h3 class="font-semibold text-gray-800">Report #{{ $report->id }}</h3>
                            <p class="text-sm text-gray-500">
                                {{ $report->created_at ? $report->created_at->format('M d, Y') : 'N/A' }}</p>
                        </div>
                    </div>
        ...
        <div class="mt-8 text-center">
            <div class="inline-flex items-center px-6 py-3 bg-blue-50 rounded-lg">
                ...
                <span class="text-blue-800">
                    Total {{ $reports->count() }} {{ $reports->count() === 1 ? 'report' : 'reports' }} found for this
                    mobile number
                </span>
            </div>
        </div>
